...
const main = (data) => {
    ...
    const reg = /(?:(?:31(\/|-|\.|\s)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.|\s)(?:0?[13-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})|(?:29(\/|-|\.|\s)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))|(?:0?[1-9]|1\d|2[0-8])(\/|-|\.|\s)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{2})/g
    ...
    if (data.match(reg)) {
        ...
        const toNumeric = ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec']
        let foundMatch = data.match(reg2)[0]
        foundMatch = foundMatch.split(/\/|\.|\s/g)
        // console.log(foundMatch)
        fullDate = new Date(foundMatch[2], toNumeric.indexOf(foundMatch[1].toLowerCase()), foundMatch[0])
    } else if (data.match(reg3)) {
        let foundMatch = data.match(reg3)[0]
        foundMatch = foundMatch.split(/\/|\.|\s/g)
        fullDate = new Date(Number('20' + foundMatch[2]), foundMatch[1] - 1, foundMatch[0])
    }
