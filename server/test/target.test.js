...
beforeAll(done => {
    ...
    .then(user => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            target.startDate = sqlDateFormat(randomDate(new Date(2012, 0, 1), new Date()))
        ...
        activeTarget.startDate = sqlDateFormat(randomDate(new Date(2012, 0, 1), new Date()))
        ...
        .end((err, res) => {
            if (err) {
                ...
                for (let i = 0; i < res.body.length; i++) {
                    ...
                    expect(res.body[i]).toHaveProperty('monthlyTarget', expect.any(Number))
...
describe('Add Target - FAILED', () => {
    test('Add target failed due to active target already exist', (done) => {
        let newTarget = {}
        newTarget.startDate = sqlDateFormat(randomDate(new Date(2012, 0, 1), new Date()))
        ...
        .end((err, res) => {
            if (err) {
                ...
                expect(res.body.message).toBe("Can't add new target: there is already an active target")
                done()
            }
        })
    })
